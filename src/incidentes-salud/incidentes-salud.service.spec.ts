...
const incidente: IncidenteSalud = {
  ...
  severidad: 'ALTA',
  estado: 'ABIERTO',
  titulo: 'Caída durante visita',
  descripcion: 'Paciente se cayó',
  pacienteId: 'p-2222',
  visitaId: 'v-3333',
  alertaId: null,
  profesionalSaludId: 'prof-4444',
  responsableUsuarioId: null,
  origen: 'WEB',
  externalIncidentId: null,
  metadata: {},
  creadoPorUsuarioId: 'u-1111',
...
describe('IncidentesSaludService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: AuditoriasService, useValue: { registrar: jest.fn() } },
        {
          ...
          useValue: { enviarIncidente: jest.fn().mockResolvedValue(true) },
        },
        {
          provide: CrmService,
          useValue: {
            debeEnviarTicket: jest.fn((inc: any) =>
              ['WEB', 'APP'].includes((inc?.origen ?? '').toUpperCase()),
            ),
            buildPayloadFromIncidente: jest.fn().mockReturnValue({}),
            crearTicket: jest
              .fn()
              .mockResolvedValue({ ticket: { id: 'crm-123' } }),
            extractTicketId: jest.fn(
              (response) => response?.ticket?.id ?? null,
            ),
        ...
        },
        {
          ...
          useValue: {
            findOne: jest
              .fn()
              .mockResolvedValue({ id: 'p-2222', nombres: 'Test' }),
          },
        },
      ],
    }).compile();
  ...
  it('findOne retorna el incidente si existe', async () => {
    ...
    expect(repository.findOne).toHaveBeenCalledWith({
      where: { id: 'inc-1111', deletedAt: IsNull() },
    });
  });
  ...
  it('create reporta TODO incidente MANUAL al Proyecto 11 forzado (sin importar la severidad)', async () => {
    ...
    await service.create(dto, 'u-1111');
    expect(incidentesServiceMock.enviarIncidente).toHaveBeenCalledWith(
      incidenteCritico,
      { forzar: true },
    );
  });
  ...
  it('update modifica el incidente', async () => {
    ...
    const result = await service.update('inc-1111', {
      estado: 'EN_REVISION',
    });
  ...
  it('update que CAMBIA el estado re-envía a Proyecto 11 (para cerrar/actualizar el ticket)', async () => {
    ...
    await service.update('inc-1111', { estado: 'RESUELTO' });
    expect(incidentesServiceMock.enviarIncidente).toHaveBeenCalledWith(
      resuelto,
    );
  });
  ...
  it('update que NO cambia el estado no re-envía a Proyecto 11', async () => {
    ...
    await service.update('inc-1111', { titulo: 'Nuevo título' });
