...
export class GoogleCalendarClientService {
  ...
  async exchangeCode(code: string): Promise<GoogleTokenResponse> {
    return this.postForm<GoogleTokenResponse>(
  ...
  async getUserInfo(accessToken: string): Promise<GoogleUserInfo> {
    return this.requestJson<GoogleUserInfo>(
  ...
  private async requestJson<T>(
    ...
    if (!response.ok) {
      throw new ServiceUnavailableException(
        `Google Calendar respondio ${response.status}: ${text.slice(0, 300)}`,
      );
    }
  ...
  private requireConfig(name: string): string {
    ...
    if (!value)
      throw new ServiceUnavailableException(`${name} no esta configurada.`);
    return value;
  }
}
