  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  onModuleInit() {
    if (
      this.configService.get<string>('GOOGLE_CALENDAR_AUTO_RETRY_ENABLED') !==
      'true'
    )
      return;
    ...
    const intervalMs = Number(
      this.configService.get<string>(
        'GOOGLE_CALENDAR_AUTO_RETRY_INTERVAL_MS',
      ) ?? 300000,
    );
  ...
  async syncUpdatedVisit(
    visita: Visita,
    previousProfesionalSaludId?: string,
  ): Promise<void> {
    if (
      previousProfesionalSaludId &&
      previousProfesionalSaludId !== visita.profesionalSaludId &&
      visita.googleCalendarEventId
    ) {
    ...
    await this.syncVisit(
      visita,
      visita.googleCalendarEventId ? 'UPDATE' : 'CREATE',
    );
  }
  ...
  async retryPendingVisits(
    ...
    try {
      const visits = await this.visitasRepo
        ...
        .andWhere('visita.google_calendar_sync_status IN (:...statuses)', {
      ...
      for (const visit of visits) {
        ...
        if (
          result.googleCalendarSyncStatus === 'SYNCED' ||
          result.googleCalendarSyncStatus === 'DELETED'
        )
          synced += 1;
  ...
  buildEventPayload(
    visita: Visita,
    context: VisitCalendarContext = {},
  ): GoogleCalendarEventPayload {
    const timeZone =
      this.configService.get<string>('GOOGLE_CALENDAR_DEFAULT_TIMEZONE') ||
      'America/Santiago';
    const startTime = normalizeTime(visita.horaProgramada);
    const end = addMinutes(
      visita.fechaProgramada,
      startTime,
      visita.duracionEstimadaMin ?? 60,
    );
    ...
    return {
      ...
      description: [
        ...
        context.pacienteTelefono
          ? `Telefono paciente: ${context.pacienteTelefono}`
          : null,
  ...
  private async syncVisit(visita: Visita, action: SyncAction): Promise<Visita> {
    ...
    const payload =
      action === 'DELETE'
        ? null
        : this.buildEventPayload(visita, await this.getVisitContext(visita));

    try {
      const accessToken = await this.getFreshAccessToken(connection);
      let response:
        | { id?: string; etag?: string; htmlLink?: string }
        | undefined;

      if (action === 'CREATE' && payload) {
        response = await this.googleClient.createEvent(
        ...
        response = await this.googleClient.updateEvent(
          connection.calendarId,
          visita.googleCalendarEventId,
          accessToken,
          payload,
      ...
      visita.googleCalendarEventEtag =
        response?.etag ?? visita.googleCalendarEventEtag ?? null;
      visita.googleCalendarHtmlLink =
        response?.htmlLink ?? visita.googleCalendarHtmlLink ?? null;
      visita.googleCalendarSyncStatus =
        action === 'DELETE' ? 'DELETED' : 'SYNCED';
      visita.googleCalendarLastSyncAt = new Date();
      visita.googleCalendarLastError = null;
      visita.googleCalendarSyncAttempts =
        (visita.googleCalendarSyncAttempts ?? 0) + 1;
      ...
      visita.googleCalendarLastError =
        error instanceof Error
          ? error.message
          : 'Error desconocido sincronizando Google Calendar';
      visita.googleCalendarSyncAttempts =
        (visita.googleCalendarSyncAttempts ?? 0) + 1;
      const saved = await this.visitasRepo.save(visita);
  ...
  private async deleteExistingEvent(visita: Visita): Promise<void> {
    const connection = visita.googleCalendarConnectionId
      ? await this.connectionsRepo.findOne({
          where: { id: visita.googleCalendarConnectionId, deletedAt: IsNull() },
        })
      : null;
  ...
  ): Promise<ProfesionalGoogleCalendarConnection | null> {
  ...
  private async persistTokenRefresh(
    connection: ProfesionalGoogleCalendarConnection,
    token: GoogleTokenResponse,
    existingRefreshToken?: string,
  ): Promise<void> {
    const encrypted = this.tokenEncryption.encrypt(
      JSON.stringify({
  ...
  private decryptTokenBundle(connection: ProfesionalGoogleCalendarConnection): {
    accessToken: string;
