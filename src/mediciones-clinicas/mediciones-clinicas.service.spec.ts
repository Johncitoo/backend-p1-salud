...
describe('MedicionesClinicasService', () => {
  ...
  describe('findAll with filters', () => {
    it('applies pacienteId filter', async () => {
      ...
      expect(mockQB.andWhere).toHaveBeenCalledWith('mc.paciente_id = :pacienteId', { pacienteId: 'p1' });
    });
    ...
    it('applies codigoVariable filter with join', async () => {
      ...
      expect(mockQB.andWhere).toHaveBeenCalledWith('vc.codigo = :codigo', { codigo: 'PA' });
    });
  });

  describe('create', () => {
    it('creates medicion with MANUAL origin by default', async () => {
      const dto: CreateMedicionClinicaDto = { pacienteId: 'p1', variableClinicaId: 'v1', valorNumero: 120 };
      (repo.create as jest.Mock).mockReturnValue({ id: '1', ...dto, origen: 'MANUAL' });
      (repo.save as jest.Mock).mockResolvedValue({ id: '1', ...dto, origen: 'MANUAL' });
  ...
  describe('findByVisita / findByPaciente', () => {
    it('delegates to findAll with visitaId', async () => {
      ...
      expect(mockQB.andWhere).toHaveBeenCalledWith('mc.visita_id = :visitaId', { visitaId: 'v1' });
    });
  ...
  describe('remove', () => {
    it('soft-deletes medicion', async () => {
      (repo.findOne as jest.Mock).mockResolvedValue({ id: '1', deletedAt: null });
      (repo.save as jest.Mock).mockResolvedValue({ id: '1', deletedAt: new Date() });
