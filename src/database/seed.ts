...
async function seedProfesional(
  ...
  if (!usuario) {
    throw new Error(
      'No se pudo crear el usuario de seed (¿existe el rol PROFESIONAL en la tabla roles?)',
    );
  }
...
async function seedPaciente(
  ...
  const paciente =
    existente ?? (await pacientesService.create({ rut, nombres, apellidos }));

  const direcciones = await pacientesService.findDirecciones(paciente.id);
  const direccionPrincipal =
    direcciones.find((d) => d.esPrincipal) ??
    direcciones[0] ??
    (await pacientesService.createDireccion({
...
async function seedPlantilla(
  ...
  const variable =
    (await variablesClinicasService.findByCodigo(codigoVariable)) ??
  ...
  let plantilla = plantillasExistentes.find(
  ...
  const campos = await plantillasFichaService.findCamposByPlantilla(
    plantilla.id,
  );
  if (campos.length === 0) {
...
async function main() {
  ...
  try {
    ...
    const pacientesRepo = app.get<Repository<Paciente>>(
    ...
    const { usuario, profesional } = await seedProfesional(
      usuariosService,
      profesionalesService,
    );
    console.log(
      `Profesional listo: usuarioId=${usuario.id} profesionalSaludId=${profesional.id} identityUserId=${SEED_PROFESIONAL_IDENTITY_USER_ID}`,
    );
    ...
    console.log(
      `Pacientes listos: ${pacientes.map((p) => p.paciente.rut).join(', ')}`,
    );

    const plantilla = await seedPlantilla(
      variablesClinicasService,
      plantillasFichaService,
    );
    console.log(`Plantilla lista: ${plantilla.codigo} (${plantilla.id})`);

    const creadas = await seedVisitas(
      visitasService,
      visitasRepo,
      usuario.id,
      profesional.id,
    ...
    console.log(
      `Visitas de hoy: ${creadas} (o ya existían para este profesional)`,
    );
  } finally {
    await app.close();
  }
