...
async function checkExternalDependencies(configService: ConfigService) {
  ...
  const projects = [
    {
      name: 'Proyecto 6 (Notificaciones)',
      url: configService.get<string>('NOTIFICATIONS_URL'),
    },
    { name: 'Proyecto 7 (CRM)', url: configService.get<string>('CRM_API_URL') },
    { name: 'Proyecto 8 (IoT)', url: configService.get<string>('IOT_API_URL') },
    {
      name: 'Proyecto 9 (Analytics)',
      url: configService.get<string>('ANALYTICS_URL'),
    },
    {
      name: 'Proyecto 11 (Incidentes)',
      url:
        configService.get<string>('INCIDENTES_API_URL') ||
        'https://proyecto11-mochicode.onrender.com',
    },
  ];
  ...
  for (const proj of projects) {
    if (!proj.url) {
      logger.warn(
        `[${proj.name}] NO CONFIGURADO (Falta URL en variables de entorno)`,
      );
      continue;
    }
    ...
    try {
      ...
      await fetch(baseUrl, { method: 'GET', signal: controller.signal }).catch(
        () => {},
      ); // Ignoramos si da 404 o 401, lo importante es que responda
