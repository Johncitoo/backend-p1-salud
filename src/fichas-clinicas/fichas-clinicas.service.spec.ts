...
const _mockQb = () => {
  ...
  qb.getRawOne = jest
    .fn()
    .mockResolvedValue({ pacienteId: 'p-mock', count: '0' });
...
const mockPacienteAccess = () =>
  ...
  }) as unknown as PacienteAccessService;
...
describe('FichasClinicasService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(MedicionClinica),
          useValue: medicionesRepo,
        },
        ...
        { provide: VariablesClinicasService, useValue: variables },
        {
          ...
          useValue: { sendFichaUpsertEvent: jest.fn() },
        },
  ...
  describe('create', () => {
    ...
    it('creates ficha and triggers syncMediciones when plantilla is provided', async () => {
      fichasRepo.create.mockReturnValue({
        id: 'f1',
        ...dto,
        estado: 'BORRADOR',
      });
      fichasRepo.save.mockResolvedValue({
        id: 'f1',
        visitaId: 'visita-1',
        contenido: dto.contenido,
        estado: 'BORRADOR',
        plantillaFichaId: null,
      });
    ...
    it('validates plantillaFichaId exists when provided', async () => {
      (plantillas.findOne as jest.Mock).mockRejectedValue(
  ...
  describe('update', () => {
    it('updates ficha fields and syncs mediciones if contenido changes', async () => {
      ...
      fichasRepo.findOne.mockResolvedValue(existing);
      fichasRepo.save.mockResolvedValue({
        ...existing,
        contenido: { temp: 37 },
        estado: 'BORRADOR',
  ...
  describe('syncMediciones', () => {
    ...
    it('soft-deletes old mediciones and recreates from content', async () => {
      const ficha = {
        id: 'f1',
        visitaId: 'v1',
        plantillaFichaId: 'pt1',
        contenido: {
          motivo_atencion: 'Control de rutina',
          temperatura: 36.8,
          presion_arterial_sistolica: 120,
        },
        creadaPorUsuarioId: 'user-1',
        version: 1,
        estado: 'BORRADOR',
        createdAt: new Date(),
        updatedAt: new Date(),
      ...
      const camposVariables = [
        {
          ...
          variableClinicaId: 'var-temp',
          tipoCampo: 'VARIABLE_CLINICA',
        },
        {
          id: 'c2',
          codigoCampo: 'presion_arterial_sistolica',
          variableClinicaId: 'var-pa',
          tipoCampo: 'VARIABLE_CLINICA',
        },
      ];

      (
        plantillas.findCamposVariablesByPlantilla as jest.Mock
      ).mockResolvedValue(camposVariables);
      (variables.findOne as jest.Mock).mockImplementation(
        async (id: string) => {
          if (id === 'var-temp')
            return {
              id: 'var-temp',
      ...
      medicionesRepo.createQueryBuilder.mockReturnValue(mockUpdateQB);
      ...
      const mockVisitaQB = {
        ...
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
        getRawOne: jest.fn().mockResolvedValue({ pacienteId: 'pac-1' }),
      };
      fichasRepo.manager.createQueryBuilder.mockReturnValue(mockVisitaQB);

      // Mock create y save de mediciones
      medicionesRepo.create.mockImplementation(
        (obj: Record<string, unknown>) => ({
          id: `med-${obj.variableClinicaId}`,
          ...obj,
        }),
      );
      medicionesRepo.save.mockImplementation(
        async (obj: Record<string, unknown>) => ({ id: obj.id, ...obj }),
      );
    ...
    it('throws BadRequestException when valor is out of range', async () => {
      ...
      await expect(service.syncMediciones(ficha)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('findAll', () => {
    it('filters by visitaId and estado', async () => {
      ...
      fichasRepo.createQueryBuilder.mockReturnValue(mockQB);

      await service.findAll({ visitaId: 'v1', estado: 'BORRADOR' });
      expect(mockQB.andWhere).toHaveBeenCalledWith('fc.visita_id = :visitaId', {
        visitaId: 'v1',
      });
      expect(mockQB.andWhere).toHaveBeenCalledWith('fc.estado = :estado', {
        estado: 'BORRADOR',
      });
    });
  });
