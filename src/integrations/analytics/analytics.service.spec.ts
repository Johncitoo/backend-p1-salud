...
describe('AnalyticsService', () => {
  ...
  describe('modo mock (ANALYTICS_ENABLED=false)', () => {
    it('loguea zona_upsert sin hacer fetch', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('[Analytics mock] Evento zona_upsert'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"zona_id": "z-1111"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"nombre": "Zona Norte"'),
      );
    });
    ...
    it('loguea usuario_upsert con campos correctos', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"usuario_id": "u-1111"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"nombres": "Maria"'),
      );
    });
    ...
    it('loguea paciente_upsert con fecha_nacimiento formateada', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"paciente_id": "p-2222"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"fecha_nacimiento": "1985-04-12"'),
      );
    });

    it('loguea profesional_upsert con nombres del usuario', async () => {
      await service.sendProfesionalUpsertEvent(profesional, {
        nombres: 'Maria',
        apellidos: 'Perez',
      });

      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"profesional_id": "prof-3333"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"nombres": "Maria"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"usuario_id": "u-4444"'),
      );
    });
    ...
    it('loguea especialidad_upsert', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"especialidad_id": "esp-5555"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"nombre": "Cardiologia"'),
      );
    });
    ...
    it('loguea visita_upsert con estado normalizado', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"estado": "programada"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"completada": 0'),
      );
    });
    ...
    it('loguea visita_upsert con estado completada y puntual', async () => {
      const completada = {
        ...visita,
        estado: 'REALIZADA',
        fechaHoraFinReal: new Date(),
      } as Visita;
      await service.sendVisitUpsertEvent(completada, { puntual: true });

      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"estado": "completada"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"completada": 1'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"puntual": 1'),
      );
    });
    ...
    it('loguea visita_inicio con fecha_inicio_real', async () => {
      const iniciada = {
        ...visita,
        fechaHoraInicioReal: new Date('2026-07-01T10:35:00Z'),
      } as Visita;
      await service.sendVisitaInicioEvent(iniciada);

      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"visita_id": "v-6666"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          '"fecha_inicio_real": "2026-07-01T10:35:00.000Z"',
        ),
      );
    });
    ...
    it('loguea visita_fin con datos de cierre', async () => {
      const finalizada = {
        ...visita,
        estado: 'REALIZADA',
        fechaHoraFinReal: new Date('2026-07-01T11:00:00Z'),
      } as Visita;
      await service.sendVisitaFinEvent(finalizada, { puntual: true });

      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"visita_fin"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"completada": 1'),
      );
    ...
    it('loguea ficha_upsert con estado mapeado BORRADOR→DRAFT', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"estado": "DRAFT"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"tiene_adjuntos": "0"'),
      );
    });

    it('loguea ficha_upsert con estado CERRADA→COMPLETED y adjuntos', async () => {
      const cerrada = { ...ficha, estado: 'CERRADA' };
      await service.sendFichaUpsertEvent(cerrada, 3);

      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"estado": "COMPLETED"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"tiene_adjuntos": "1"'),
      );
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"cantidad_adjuntos": "3"'),
      );
    });

    it('loguea ficha_upsert con estado ANULADA→ARCHIVED', async () => {
      const anulada = { ...ficha, estado: 'ANULADA' };
      await service.sendFichaUpsertEvent(anulada, 0);

      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('"estado": "ARCHIVED"'),
      );
    });
  });
  ...
  describe('modo real (ANALYTICS_ENABLED=true)', () => {
    ...
    it('advierte si URL está vacía con ENABLED=true', async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining('ANALYTICS_URL está vacío'),
      );
    });
  });
  ...
  describe('normalización de estados de visita', () => {
    ...
    test.each(cases)('estado %s → %s', async (input, expected) => {
      const v = { ...visita, estado: input };
      await service.sendVisitUpsertEvent(v);

      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining(`"estado": "${expected}"`),
      );
    });
  });
});
